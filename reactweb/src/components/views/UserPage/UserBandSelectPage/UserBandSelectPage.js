    ...
    if( band == 'DL' ){
      if( number < numberOfChannels){
        ...
        if( addr > 0){
          ...
          if( bandSelect === 'ps700' ){ DLBeforMax = 768.000000 - 0.000001; }
          else{ DLBeforMax = 851.000000 - 0.000001; }
        }
        ...
        DLmin = Number(e.currentTarget.value) - Number(channelData[addr].BW/2);
        DLBeforMax = Number(channelData[addr-1].DLfreqInfo) + Number(channelData[addr-1].BW/2)
        if( bandSelect == 'ps700' ){  DLAftermin = 776.000000-0.000001; }
        else{ DLAftermin = 869.000000-0.000001; }
      }
      ...
      if( number < numberOfChannels){
        ...
        if(addr > 0){
          ...
          if( bandSelect === 'ps700' ){ ULBeforMax = 798.000000 - 0.000001; }
          else{ ULBeforMax = 806.000000 - 0.000001; }
        }
      }else{
        ULMax = Number(e.currentTarget.value) + Number(channelData[addr].BW/2);
        ULmin = Number(e.currentTarget.value) - Number(channelData[addr].BW/2);
        ULBeforMax = Number(channelData[addr].ULfreqInfo) + Number(channelData[addr].BW/2)  
        if( bandSelect == 'ps700' ){ ULAftermin = 806.000000-0.000001; }
        else{ ULAftermin = 824.000000-0.000001; }
      }
        ...
        <div className='w-full h-userMain font-sans font-bold text-stone-700'>
          <div className='flex h-1/16'>
            <div className='w-1/6 h-full inline-block flex items-center'>
              <select className='w-28 h-8/10 text-center text-lg border-2 border-gray-500 rounded-lg' disabled={autoRefresh? true:false} onChange={classChange}>
          ...
          <div className='w-full h-1/10 flex'>
            <div className='w-1/6 h-full inline-block align-bottom flex items-center'>
              <select className='w-28 h-1/2 text-center text-lg border-2 border-gray-500 rounded-lg' disabled={autoRefresh? true:false} onChange={BandSelectChange}>
